...
print(x)
#Y计算比率
y = data.iloc[:,2].values / data.iloc[:,1].values ;
y.shape = len(y),1
...
hidden_bias = tf.Variable(np.random.rand(hidden_num))
output_bias = tf.Variable(np.random.rand(output_num))
#
...
for i in range(迭代次数):
    ...
    if i%100 == 0:
        ...
        ax.plot(tempY[:,0],'r*',label='实际数据')
        ax.plot(y_fits[:,0] , 'b+',label='预测数据')
        ...
        print(tempY[:10,0])
        print(y_fits[:10,0])
        print(sess.run(hidden_bias))
        print('+++++++++++++++++++++++++')

# plot   根据输入数据特点修改
y_fits = sess.run(y_fit, feed_dict={x_train: tempX})
ax.plot(tempY[:, 0], 'r*',name='实际数据')
ax.plot(y_fits[:, 0], 'b+',name='预测数据')
